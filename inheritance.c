...
person *create_family(int generations)
{
    ...
    person *children = malloc(sizeof(person));
    if (children == NULL)
    {
    ...
    if (generations > 1)
    {
        ...
        children->alleles[0] = random_parent_allele(parent0);
        children->alleles[1] = random_parent_allele(parent1);
...
// Randomly chooses one of the parents alleles
char random_parent_allele(person *parent)
{
    int r = rand() % 2;
    if (r == 0)
